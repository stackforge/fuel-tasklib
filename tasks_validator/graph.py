...
class DeploymentGraph(nx.DiGraph):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # We need check if nodes are not initialize before to not
        # overwrite them. Some of the method of networkx copy
        # graph before operations. So to be compatible we need only
        # initialize self.node only first time
        if not self.node:
